...
console.log('Seating users... (hybrid concurrency control)')
...
async function seatUser(userId: number): Promise<number | null> {
  ...
  try {
    // start a transaction (default isolation level is `READ COMMITTED`).
    ...
    const result = await client.query('select id, version from "Seat" where "userId" is null limit 1 FOR UPDATE;')
    ...
    await client.query('update "Seat" set "userId" = $1, "version" = "version" + 1 where id = $2 and "version" = $3', [
      userId,
      seatId,
      version,
    ])
    ...
    console.log(JSON.stringify({ seatId, version, userId }))
